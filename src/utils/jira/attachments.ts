...
export async function uploadAttachment(
  ...
  try {
    ...
    if (attachmentResponse.ok) {
      ...
      console.error(
        `❌ Failed to upload ${fileName}: ${attachmentResponse.status} - ${errorText}`
      );
      return false;
    }
  } catch (attachError) {
...
export async function getLatestAttachments(
  testInfo: any
): Promise<{ attachments: any[]; outputDir: string }> {
  console.log('Getting latest attachments from testInfo');
  console.log(
    'Current attachments:',
    JSON.stringify(testInfo.attachments, null, 2)
  );
