...
export default function EventRecorderApp() {
  ...
  useEffect(() => {
    const handleClick = (e: MouseEvent) => {
      ...
      if (el) {
        ...
        const hasHref =
          target.hasAttribute('href') || !!target.closest('[href]');
        const isButtonWithNavigation =
    ...
    const handleKeydown = (e: KeyboardEvent) => {
      ...
      console.log(
        '[EVENT RECORDER] Input/Select keyboard interaction detected'
      );
  ...
  const handleContextMenuAction = (action: string, element: HTMLElement) => {
    ...
    if (action === 'click') {
      ...
      const hasHref =
        element.hasAttribute('href') || !!element.closest('[href]');
      const isButtonWithNavigation =
  ...
  return (
    <Card className="mx-auto p-3 inline-flex flex-row items-center absolute top-2 left-2 z-1000">
      ...
      />
      <GeneratedTestModal recordedElements={recordedElements} />
